...
export default function Mainheader(){
    ...
    return (
        ...
        <div className='md:flex items-center justify-between bg-yellow-400 py-4 md:px-10 px-7' >
            ...
            <button onClick={() => setOpen(!Open)} className='absolute right-8 top-8 cursor-pointer md:hidden' >
                {Open ? <X /> : <Menu />}
            </button>

      
            <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-yellow-400 md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${Open ? "top-30 opacity-100" : "top-[-498px]"} md:opecity-100 opecity-0 `}>
                {navlinks.map((links) => (
                    <li key={links.name} className='md:ml-8 text-xl md:my-0 my-7  md:p-8'>
                ...
                <button className='p-4 py-3  md:ml-8  cursor-pointer duration-200'>
                    ...
                    <Link href="/cart">
                    <button className='md:ml-10 cursor-pointer relative inline-block' onClick={openCart} aria-label='open cart'>
