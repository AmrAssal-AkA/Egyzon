...
export const NotificationServices = {
  ...
  notifyPartenerApplicantsToAdmin: async (
    notificationData: PartnerApplicant,
  ) => {
    try {
      ...
      logger.error(
        "[notifyPartenerApplicantsToAdmin] Unexpected error:",
        error,
      );
      throw new AppError(500, "Internal Server Error");
    }
  },
  notifySellerAdminForNewOrder: async (notificationData: {
    orderId: string;
    sellerId: string;
    buyerName: string;
  }) => {
    try {
      ...
      const notificationsPromises = [...adminUser, sellerUser].map(
        async (user) => {
          const notification = new Notification({
            type: "new_order",
            message: messages,
