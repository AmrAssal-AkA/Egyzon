...
exports.userBaseSchema = zod_1.z.object({
    FirstName: safeTextSchema.min(1, { message: "First name is required" }).transform(senitize_1.sentizePlainText),
    LastName: safeTextSchema.min(1, { message: "Last name is required" }).transform(senitize_1.sentizePlainText),
...
exports.LoginSchema = zod_1.z.object({
    body: zod_1.z.object({
        email: zod_1.z.string().email().transform(senitize_1.sentizePlainText),
        password: zod_1.z.string().min(1, 'Password is required').transform(senitize_1.sentizePlainText),
    })
});
exports.userResposnseSchema = zod_1.z.object({
    role: zod_1.z.enum(['customer', 'seller', 'admin']),
    address: zod_1.z.array(safeTextSchema).transform((address) => address.map(senitize_1.sentizePlainText)),
    phoneNumber: zod_1.z.array(phoneNumberSchema).transform((phoneNumbers) => phoneNumbers.map((phone) => phone)),
