...
export default function SellerHeader() {
  ...
  return (
    <>
      <header className="w-full py-3.5 px-4 sm:px-6 lg:px-8 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 text-slate-900 dark:text-white rounded-2xl shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 transition-colors duration-200">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 self-end sm:self-auto">
          ...
          >
            {user?.image ? (
              ...
              <UserCircleIcon className="w-6 h-6 sm:w-7 sm:h-7" aria-hidden="true" />
            )}
          </button>
        </div>
      </header>
