...
function NavLinks({ isOpen, onLinkClick }: NavLinksProps) {
  ...
  const navLinks = [
    { id: 1, name: "Home", href: "/", icon: <FiHome className="w-4 h-4" /> },
    { id: 2, name: "Products", href: "/products", icon: <FiShoppingBag className="w-4 h-4" /> },
    { id: 3, name: "Categories", href: "/categories", icon: <FiGrid className="w-4 h-4" /> },
    { id: 4, name: "About Us", href: "/about", icon: <FiInfo className="w-4 h-4" /> },
    { id: 5, name: "Contact", href: "/contact", icon: <FiPhoneCall className="w-4 h-4" /> },
    { id: 6, name: "Become a Seller", href: "/Partner", icon: <FiBriefcase className="w-4 h-4" /> },
  ];
    ...
    <>
      ...
      <div className="hidden md:block w-full bg-secondary/80 backdrop-blur border-b border-border shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          <nav className="flex items-center gap-1 py-2 text-sm font-medium">
            {navLinks.map((link) => {
