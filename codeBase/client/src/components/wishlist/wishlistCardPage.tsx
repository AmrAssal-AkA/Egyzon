...
export default function WishlistClientComponent() {
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center border border-dashed border-border rounded-2xl bg-muted/20">
          ...
          <p className="text-muted-foreground text-sm mt-2 max-w-md px-4">
            Explore our catalog and click the heart button on items you like to
            save them for later!
          </p>
          <Link
            href="/"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div
              key={item.id}
              className="bg-card text-card-foreground shadow-md hover:shadow-lg rounded-lg overflow-hidden border border-border flex flex-col justify-between h-96 transition-all duration-300"
            >
              {/* Product Image */}
