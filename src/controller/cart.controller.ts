...
async function createCart(req: Request, res: Response) {
    try {
        const userId = req.user?.userId as string || (process.env.NODE_ENV !== 'production' && req.body.userId);
        ...
        if (existingCartId) {
            return sendErrorResponse(res, 409, 'Cart already exists for this user', 'Cart already exists for this user');
        }
...
const getCart = async (req: Request, res: Response) => {
    try {
        ...
        if (!cartId) {
            return sendErrorResponse(res, 404, 'Cart not found for this user', 'Cart not found for this user');
        }
